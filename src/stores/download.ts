  ...
  actions: {
    ...
    updateDownload(id: string, updates: Partial<DownloadState>): void {
      const updateEntry = (downloads: Record<string, DownloadState>) => {
        ...
        if ((updates.status === 'completed' || updates.status === 'error') && !updated.endTime) {
    ...
    removeDirectDownloadFromList(id: string): void {
      const download = Object.values(this.directDownloads).find((download) => download.id === id)
    ...
    processQueue(): DownloadItem | null {
      ...
      if (this.activeDownloads.length >= this.configuration.maxConcurrent) return null
    ...
    getWorkerByDownloadId(downloadId: string): ManagedWorker | null {
      const worker = this.activeWorkers.find((w) => w.download.id === downloadId)
      if (!worker) {
        return null
      }
      return worker
